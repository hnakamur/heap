...
func (h *MaxInt64) verify(t *testing.T, i int) {
	t.Helper()
	n := h.length()
	j1 := 2*i + 1
	j2 := 2*i + 2
...
func TestMaxInt64Init0(t *testing.T) {
	h := new(MaxInt64)
	for i := 20; i > 0; i-- {
...
func TestMaxInt64Init1(t *testing.T) {
	h := new(MaxInt64)
	for i := 20; i > 0; i-- {
...
func TestMaxInt64Remove0(t *testing.T) {
	h := new(MaxInt64)
	for i := 9; i >= 0; i-- {
		h.push(int64(i))
	}
	...
	for h.length() > 0 {
		...
		x := h.Remove(i)
		if x != int64(9-i) {
			t.Errorf("Remove(%d) got %d; want %d", i, x, int64(9-i))
		}
		h.verify(t, 0)
	}
}

func TestMaxInt64Remove1(t *testing.T) {
	h := new(MaxInt64)
	for i := 9; i >= 0; i-- {
		h.push(int64(i))
	}
	...
	for i := 0; h.length() > 0; i++ {
		x := h.Remove(0)
		if x != int64(9-i) {
			t.Errorf("Remove(0) got %d; want %d", x, int64(9-i))
		}
		h.verify(t, 0)
	}
}

func TestMaxInt64Remove2(t *testing.T) {
	N := 10
...
func BenchmarkMaxInt64Dup(b *testing.B) {
	const n = 10000
	h := make(MaxInt64, 0, n)
	for i := 0; i < b.N; i++ {
		for j := 0; j < n; j++ {
...
func TestMaxInt64Fix(t *testing.T) {
	h := new(MaxInt64)
	h.verify(t, 0)
